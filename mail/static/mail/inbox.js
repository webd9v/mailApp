...
function compose_email() {
  ...
  recipients=document.querySelector('#compose-recipients');recipients.value = '';
  subject=document.querySelector('#compose-subject');subject.value = '';
  body=document.querySelector('#compose-body');body.value = '';
  //onsubmit form
...
function load_mailbox(mailbox) {
  ...
  mailboxTitle.innerHTML=`<h3>${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)}</h3>`;
    ...
    .then(emails => {
        ...
        emails.map((email)=>{
          ...
          if(!(email["sender"]==email["user"])){
            ...
            btnReply.onclick=()=>{reply(email["sender"],email["subject"]);}
...
function reply(recipients,subjects){
  ...
  subject=document.querySelector('#compose-subject');subject.value = `RE: ${subjects}`;
}